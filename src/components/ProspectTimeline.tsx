...
export const ProspectTimeline: React.FC<Props> = ({ isOpen, onClose, prospect, followUps, onAddFollowUp, onUpdateFollowUp, onDeleteFollowUp }) => {
    ...
    <div style={{
      ...
      <div className="card animate-fade-in" onClick={(e) => e.stopPropagation()} style={{ 
        ...
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ fontSize: '1.5rem' }}>{prospect.nom_association}</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Contact : {prospect.prenom} {prospect.nom}</p>
          </div>
